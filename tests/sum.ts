...
describe("sum", () => {
  ...
  it("Is initialized!", async () => {
    ...
    const tx = await program.methods.initialize(sum_init).accounts({
    ...
    const sumAccountReturn = await program.account.sum.fetch(sumAccountKeypair.publicKey);
  ...
  it("Is updated!", async () => {
    ...
    const tx = await program.methods.update(sum_init_new).accounts({
      sumAccount: sumAccountKeypair.publicKey
    }).rpc()

    const sumAccountReturn = await program.account.sum.fetch(sumAccountKeypair.publicKey)
    expect(sumAccountReturn.value.toNumber()).to.be.equal(sum_init_new.toNumber())
  })

  it("Is increased!", async() =>{
    const delta = new anchor.BN(30)
    const sumAccountReturn1 = await program.account.sum.fetch(sumAccountKeypair.publicKey)
    const before_value = sumAccountReturn1.value.toNumber()
    // console.log("Before value", before_value)

    const tx = await program.methods.increase(delta).accounts({
      sumAccount: sumAccountKeypair.publicKey
    }).rpc()

    const sumAccountReturn2 = await program.account.sum.fetch(sumAccountKeypair.publicKey)
